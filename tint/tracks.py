...
class Tracks(object):
    ...
    def __init__(self, params={}, field="reflectivity"):
        ...
        self.params = {
            ...
            "LEVELS": np.array([[500, 3500], [3500, 7500], [7500, 10000]]),  # m
            "WIND_LEVELS": None,
    ...
    def get_suffix(self, datetime):
        ...
        if self.params["REFERENCE_RADAR"] == 31:
            if dt < np.datetime64("2011-12-01"):
                suffix = "a"
            elif dt >= np.datetime64("2011-12-01") and dt < np.datetime64("2019-05-14"):
                suffix = "b"
            else:
                suffix = "c"
            ...
            try:
                ...
                try:
                    ...
                    while len(new_file_list) < 6:
                        ...
                        temp_dt_list = sorted(extract_datetimes(new_file_list))
                        time_diffs = (np.diff(np.array(temp_dt_list))).astype(int)
                        if len(new_file_list) < 6:
                            print(
                                "Fewer than six files in new day {}, skipping.".format(
                                    new_day
                                )
                            )
                            new_file_list = []
                            continue
                        new_file_list = list(
                            np.array(new_file_list)[
                                np.array([True] + list((np.array(time_diffs) > 120)))
                            ]
                        )
                        temp_dt_list = sorted(extract_datetimes(new_file_list))
                        DT_temp = int(
                            np.argmax(
                                np.bincount(
                                    (np.diff(np.array(temp_dt_list))).astype(int)
                                )
                            )
                            / 60
                        )
                    ...
                    if self.params["DT"] < 7:
                        scale_factor = np.ceil(10 / self.params["DT"]).astype(int)
                        self.params["DT"] = self.params["DT"] * scale_factor
                        new_file_inds = [
                            i
                            for i in np.arange(0, len(dt_list))
                            if dt_list[i].astype("<M8[m]")
                            in np.arange(
    ...
    def get_next_grid(self, grid_obj2, grids, data_dic, day_grids=None):
        ...
        data = extract_grid_data(grid_obj2, self.field, self.grid_size, self.params)
        data_names = ["refl", "rain", "frames", "cells", "stein_class"]
        for i in range(len(data_names)):
            data_dic[data_names[i] + "_new"] = data[i]
        # Skip grids that are artificially zero
        while np.max(data_dic["refl"]) > 30 and np.max(data_dic["refl_new"]) == 0:
    ...
    def get_tracks(self, grids, day_grids=None, b_path=None):
        ...
        if self.record is None:
            ...
            self.grid_obj_day = np.datetime64(str(parse_grid_datetime(grid_obj2))[:10])
        else:
        ...
        for i in range(len(data_names)):
            data_dic[data_names[i] + "_new"] = data[i]
        data_dic["frame_new"] = data_dic["frames_new"][self.params["TRACK_INTERVAL"]]
        ...
        while grid_obj2 is not None:
            ...
            if not new_rain:
                # Set old frame equal to current frame from last iteration.
            ...
            if grid_obj2 is not None:
                ...
                if (
                    ...
                    message = "Time discontinuity at {}.".format(self.record.time)
                    print(message)
                    new_rain = True
                    self.current_objects = None
                ...
                if self.params["RESET_NEW_DAY"]:
                    ...
                    if next_day_new:
                        new_rain = True
                        print("Scan {} last before new day.".format(self.record.scan))
                        self.current_objects = None
                        data_dic["refl_new"] = np.zeros_like(data_dic["refl"])
            ...
            if self.params["AMBIENT"] == "ERA5":
                ...
                try:
                    ...
                    print("Could not load ambient winds at {}".format(current_datetime))
                    print("Skipping.")
                    new_rain = True
                    self.current_objects = None
                    continue
            ...
            pairs, obj_merge_new, u_shift, v_shift = get_pairs(
                data_dic, global_shift, self.current_objects, self.record, self.params
            )
            ...
            if new_rain:
                ...
                self.current_objects, self.counter = init_current_objects(
                    ...
                    pairs,
                    self.counter,
                    self.record.interval.total_seconds(),
                    self.params,
                )
                new_rain = False
            else:
