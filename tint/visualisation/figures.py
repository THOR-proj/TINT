...
def get_bounding_box(tracks, grid, date_time, params):
    lon_box, lat_box, x, y = vh.get_center_coords(tracks, grid, params, date_time)
...
def add_crosshair(tracks, grid, date_time, params, ax, display, lon_box, lat_box):
    ...
    if params["uid_ind"] is not None:
        ...
        if params["center_cell"] and (params["cell_ind"] is not None):
            ...
            lon_cell, lat_cell = cartesian_to_geographic(x_cell, y_cell, projparams)
    ...
    if params["line_coords"]:
        ...
        ax.plot(
            [0, 360], [crosshair_lat_0, crosshair_lat_1], "--r", linewidth=2.0, zorder=2
        )
        ...
        display.plot_crosshairs(
            lon=crosshair_lon, lat=crosshair_lat, transform=ccrs.PlateCarree()
        )


def init_cross_section(tracks, grid, params, alt, fig, ax, date_time, figsize=(12, 12)):
...
def horizontal_cross_section(
    ...
    if alt == "col_max":
        col_max = np.ma.masked_invalid(grid.fields["reflectivity"]["data"].data)
    ...
    [lon_lines, lat_lines] = cartesian_to_geographic(
        np.arange(-150e3, 200e3, 75e3), np.arange(-150e3, 200e3, 75e3), projparams
    )
    ...
    gridlines.xlabel_style = {"rotation": 0, "ha": "center"}
    ...
    ocean_10m = cfeature.NaturalEarthFeature(
        "physical", "ocean", "50m", edgecolor="face", facecolor=sea_color
    )
    ax.add_feature(ocean_10m, zorder=0)

    land_10m = cfeature.NaturalEarthFeature(
        "physical", "land", "50m", edgecolor="face", facecolor=land_color
    )
    ...
    lgd_ellipse = mlines.Line2D(
        ...
        color="w",
        linewidth=ell_w,
        label="Best Fit Ellipse",
        linestyle="--",
        path_effects=[
            pe.SimpleLineShadow(shadow_color="k", alpha=0.9, linewidth=ell_w + 2),
            pe.Normal(),
        ],
    )
    ...
    if params["save_dir"] is not None:
        plt.savefig(
            "{}/{}m_cross_{}.png".format(params["save_dir"], params["alt"], date_time),
            bbox_inches="tight",
            dpi=params["dpi"],
            facecolor="w",
        )
...
def vertical_cross_section(
    ...
    if params["line_coords"]:
        ds, tmp_tracks = vh.setup_line_coords(ds, tracks, params, date_time, variables)
    ...
    if params["direction"] == "parallel":
        ...
        ds_plot, t_string = vh.setup_perpendicular_coords(ds, x, tmp_tracks, params, dx)
        extent = [
            ds_plot.y[0] / 1000,
            ds_plot.y[-1] / 1000,
            ds_plot.z[0] / 1000,
    ...
    lgd_so, so_angle = vh.add_stratiform_offset(ax, tracks, grid, date_time, params)
    ...
    ax.set_yticklabels(np.round((np.arange(0, 24, 4))))
    ...
    if params["data_fn"] is not None:
        print("Saving data.")
        save_tilt_data(
            sl_angle, w_max, n_angles, n_obs, max_count, so_angle, date_time, params
        )
        if not params["load_line_coords_winds"] and params["save_ds"]:
            fn = "{}/{}_{}.nc".format(params["save_dir"], params["uid_ind"], date_time)
            ds.to_netcdf(fn)
...
def save_tilt_data(
    ...
    try:
        dtypes = {
            "uid": str,
            "streamline_angle": float,
            "stratiform_offset_angle": float,
            "w_max": float,
            "n_angles": float,
            "n_obs": float,
            "max_count": float,
...
def two_level(tracks, grid, params, date_time=None, alt1=None, alt2=None):
    ...
    horizontal_cross_section(
        tracks, grid, params=tmp_params, alt=alt1, fig=fig, ax=ax, date_time=date_time
    )
    ...
    horizontal_cross_section(
        tracks, grid, params=tmp_params, alt=alt2, fig=fig, ax=ax, date_time=date_time
    )
...
def object(tracks, grid, params, date_time=None, alt=None):
    ...
    horizontal_cross_section(tracks, grid, params, alt, fig, ax, date_time)

    ax = fig.add_subplot(2, 2, 3, projection=projection)
    horizontal_cross_section(tracks, grid, params, 9000, fig, ax, date_time)
...
def concat_angles(base_dir, save_path, max_uid):
    data = pd.read_csv("{}/vertical_cross_section_0_frames/angles.csv".format(base_dir))
    for i in range(1, max_uid + 1):
        new_data = pd.read_csv(
            "{}/vertical_cross_section_{}_frames/angles.csv".format(base_dir, i)
        )
...
def get_angle_props(angles, tracks_obj):
    angle_diff = np.abs(angles["streamline_angle"] - angles["stratiform_offset_angle"])
...
def angle_correlation(
    sl_angles, so_angles, cond, save_path, fig=None, ax=None, title="Angle Correlations"
):
    ...
    ax.scatter(sl_angles[~cond], so_angles[~cond], marker="x", color="gray", s=40)
    ax.scatter(
        sl_angles[cond], so_angles[cond], marker="o", color="r", s=50, edgecolor="k"
    )
    ...
    ax.text(0.5, 1.02, title, ha="center", transform=ax.transAxes, size=12)
