...
def get_radar_info(grid_obj):
    ...
    info = {"radar_lon": radar_lon, "radar_lat": radar_lat}
    return info
...
def get_filtered_frame_steiner(
    grid_obj, field, grid_size, min_size, z_min=None, z_max=None
):
    """Returns a labeled frame from gridded radar data. Smaller objects
    are removed and the rest are labeled."""
    ...
    try:
        sclass = steiner_conv_strat(
            grid, grid_obj.x["data"], grid_obj.y["data"], grid_size[1], grid_size[2]
        )
    except:
...
def get_level_indices(grid_obj, grid_size, levels):
    ...
    [z_min, z_max] = [get_grid_alt(grid_size, level) for level in levels]
...
def get_connected_components(frames):
    ...
    for i in range(frames.shape[0] - 1):
        ...
        for j in range(len(list(objects))):
            overlap = np.logical_and(frames[i] == list(objects)[j], frames[i + 1] > 0)
...
def extract_grid_data(grid_obj, field, grid_size, params):
    ...
    if params["RAIN"]:
        ...
        cond2 = np.isnan(masked_rain.data)
        masked_rain.data[cond1 | cond2] = 0
    ...
    for i in range(frames.shape[0] - 1, -1, -1):
        ...
        [z_min, z_max] = get_level_indices(grid_obj, grid_size, params["LEVELS"][i, :])
        if np.any(masked.data[z_min:z_max] > 0):
            if params["FIELD_THRESH"][i] == "convective":
                ...
                frames[i] = get_filtered_frame(
                    masked.data, min_sizes[i], params["FIELD_THRESH"][i], z_min, z_max
                )
        else:
            break
