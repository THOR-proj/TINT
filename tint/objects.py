...
def check_isolation(refl, filtered, grid_size, params, level):
    ...
    iso_filtered = get_filtered_frame(refl, min_size, params["ISO_THRESH"][level])
...
def get_ellipse(data_dic, grid1, record, obj, level_ind, u_shift, v_shift):
    ...
    contours = cv.findContours(
        hull.astype(np.uint8), cv.RETR_TREE, cv.CHAIN_APPROX_NONE
    )[0]
...
def get_object_prop(
    data_dic, grid1, u_shift, v_shift, field, record, params, current_objects
):
    ...
    for i in range(levels):
        ...
        for obj in np.arange(nobj) + 1:
            ...
            b_intersect = obj_ind_set.intersection(params["BOUNDARY_GRID_CELLS"])
            ...
            ellipse = get_ellipse(data_dic, grid1, record, obj, i, u_shift, v_shift)
            props = [
                "grid_x",
                "grid_y",
                "semi_major",
            ...
            try:
                ...
                tp_idx = (
                    data_dic["ambient_interp"]["tp_idx"]
                    .sel(
                        longitude=lon[0],
                        latitude=lat[0],
                        time=grid_time,
                ...
                freezing_level = float(
                    data_dic["ambient_interp"].altitude[fl_idx].values
                )
            ...
            if params["AMBIENT"] is not None:
                ...
                interval = np.arange(
                    ...
                    params["WIND_LEVELS"][i, 1] + unit_dim[0],
                    unit_dim[0],
                )
                ...
                obj_prop["u_ambient_mean"].append(np.nanmean(np.array(ambients_u)))
                obj_prop["v_ambient_mean"].append(np.nanmean(np.array(ambients_v)))
            ...
            if params["FIELD_THRESH"][i] == "convective":
                ...
                filtered_slices = [
                    stein_class_slice == 2 for stein_class_slice in stein_class_slices
                ]
            else:
                obj_prop["field_max"].append(np.max(obj_slices))
                filtered_slices = [
                    obj_slice > params["FIELD_THRESH"][i] for obj_slice in obj_slices
                ]
    ...
    if params["RAIN"]:
        ...
        obj_prop.update(
            {
                "tot_rain": rain_props[0],
                "max_rr": rain_props[1],
                "tot_rain_loc": rain_props[2],
                "max_rr_loc": rain_props[3],
            }
        )
...
def smooth(group_df, r=3, n=2):
    ...
    if len(group_df) >= (2 * n + 1):
        group_df_smoothed = group_df.rolling(window=(2 * n + 1), center=True).mean()
...
def post_tracks(tracks_obj):
    ...
    tracks_obj.tracks.drop(tracks_obj.tracks.index.max()[0], level="scan", inplace=True)

    tracks_obj.tracks["field_max"] = np.round(tracks_obj.tracks["field_max"], 2)
    ...
    tracks_obj.tracks[["u_shift", "v_shift"]] = tmp_tracks.apply(lambda x: fill_end(x))
    ...
    tracks_obj.tracks[["u_shift", "v_shift"]] = tmp_tracks.apply(lambda x: smooth(x))
    ...
    tmp_tracks = tmp_tracks.apply(lambda x: (x.values[4] - x.values[0]) / (4 * dt))
    ...
    tracks_obj.tracks["u_shear"] = np.round(
        tracks_obj.tracks["u_ambient_top"] - tracks_obj.tracks["u_ambient_bottom"], 5
    )
    tracks_obj.tracks["v_shear"] = np.round(
        tracks_obj.tracks["v_ambient_top"] - tracks_obj.tracks["v_ambient_bottom"], 5
    )
...
def classify_tracks(tracks_obj):
    ...
    tracks_class = copy.deepcopy(tracks_obj.tracks[["grid_x", "grid_y", "lon", "lat"]])
...
def temporal_continuity_check(
    group_df, length=np.timedelta64(30, "m"), dt=np.timedelta64(10, "m")
):
...
def get_duration_cond(tracks_obj):
    ...
    for uid in uids:
        tracks_obj.exclusions.loc[
            (slice(None), slice(None), uid, slice(None)), "duration_cond"
        ] = duration_checks.loc[uid]
...
def get_simple_duration_cond(tracks_obj):
    ...
    for uid in uids:
        tracks_obj.exclusions.loc[
            (slice(None), slice(None), uid, slice(None)), "simple_duration_cond"
        ] = duration_checks.loc[uid]
...
def get_exclusion_categories(tracks_obj):
    ...
    exclusions = copy.deepcopy(tracks_obj.tracks[["grid_x", "grid_y", "lon", "lat"]])
    tracks_obj.exclusions = exclusions

    small_area = (
        tracks_obj.system_tracks["proj_area"] < excl_thresh["SMALL_AREA"]
    ).values
    tracks_obj.exclusions["small_area"] = np.repeat(small_area, n_lvls)

    small_conv_area = (
        tracks_obj.tracks["proj_area"].xs(0, level="level") < excl_thresh["SMALL_AREA"]
    ).values
    tracks_obj.exclusions["small_conv_area"] = np.repeat(small_conv_area, n_lvls)

    large_area = (
        tracks_obj.system_tracks["proj_area"] > excl_thresh["LARGE_AREA"]
    ).values
    tracks_obj.exclusions["large_area"] = np.repeat(large_area, n_lvls)

    int_border = (
        tracks_obj.system_tracks["touch_border"]
        * cell_area
        / tracks_obj.system_tracks["proj_area"]
    ) > excl_thresh["BORD_THRESH"]
    int_border = int_border.values
    tracks_obj.exclusions["intersect_border"] = np.repeat(int_border, n_lvls)

    tmp_tracks = tracks_obj.tracks[["touch_border", "proj_area"]].xs(0, level="level")
    int_border_conv = (
        tmp_tracks["touch_border"] * cell_area / tmp_tracks["proj_area"]
    ) > excl_thresh["BORD_THRESH"]
    ...
    rel_velocity_mag = np.sqrt(
        tracks_obj.tracks["u_relative"] ** 2 + tracks_obj.tracks["v_relative"] ** 2
    )
    small_rel_vel = rel_velocity_mag < class_thresh["REL_VEL_MAG"]
    tracks_obj.exclusions["small_rel_velocity"] = small_rel_vel
    shear_mag = np.sqrt(
        tracks_obj.tracks["u_shear"] ** 2 + tracks_obj.tracks["v_shear"] ** 2
    )
...
def calc_alt_orientation(tracks_obj):
    ...
    relative_velocity = np.array(
        [tracks_obj.tracks["u_relative"], tracks_obj.tracks["v_relative"]]
    )
    cond = (
        normal[0, :] * relative_velocity[0, :] + normal[1, :] * relative_velocity[1, :]
    )
    cond = cond < 0
...
def calc_inflow_type(tracks_obj):
    ...
    vel_dir = np.arctan2(tracks_obj.tracks["v_shift"], tracks_obj.tracks["u_shift"])
    ...
    inflow_type = np.array(["Front Fed" for i in range(len(inflow_dir))], dtype=object)
    inflow_type[np.logical_or(inflow_dir.values > 135, inflow_dir.values < -135)] = (
        "Rear Fed"
    )
    inflow_type[np.logical_and(inflow_dir.values > 45, inflow_dir.values < 135)] = (
        "Parallel Fed (Right)"
    )
    inflow_type[np.logical_and(inflow_dir.values < -45, inflow_dir.values > -135)] = (
        "Parallel Fed (Left)"
...
def calc_propagation_type(tracks_obj):
    ...
    propagation_type = np.array(
        ["Down-Shear Propagating" for i in range(len(propagation_cond))], dtype=object
    )
    propagation_type[propagation_cond < 0] = "Up-Shear Propagating"
    parallel_shear = np.abs(propagation_cond / (shear_mag * rel_velocity_mag))
...
def calc_tilt_type(tracks_obj):
    ...
    tilt_type = np.array(
        ["Ambiguous (On Quadrant Boundary)" for i in range(len(tilt_dir))], dtype=object
    )
...
def calc_stratiform_type(tracks_obj):
    ...
    offset_type = np.array(
        ["Ambiguous (On Quadrant Boundary)" for i in range(len(rel_tilt_dir))],
        dtype=object,
    )
...
def get_system_tracks(tracks_obj):
    ...
    system_tracks = tracks_obj.tracks[
        [
            ...
            "v",
            "u_shift",
            "v_shift",  #'mergers', 'parent',
            "u_relative",
            "v_relative",
        ]
    ]
    system_tracks = system_tracks.xs(tracks_obj.params["TRACK_INTERVAL"], level="level")
    ...
    pos_0.rename(
        columns={"grid_x": "x_vert_disp", "grid_y": "y_vert_disp"}, inplace=True
    )
    pos_1.rename(
        columns={"grid_x": "x_vert_disp", "grid_y": "y_vert_disp"}, inplace=True
    )
    ...
    tilt_mag = np.round(
        np.sqrt(
            (system_tracks["x_vert_disp"] ** 2 + system_tracks["y_vert_disp"] ** 2)
        ),
        3,
    )
    tilt_mag = tilt_mag.rename("tilt_mag")
    system_tracks = system_tracks.merge(tilt_mag, left_index=True, right_index=True)
    ...
    rel_vel_dir = np.arctan2(system_tracks["v_relative"], system_tracks["u_relative"])
    rel_vel_dir = np.rad2deg(rel_vel_dir)
    rel_vel_dir = rel_vel_dir.rename("rel_vel_dir")
    ...
    tilt_dir = np.arctan2(system_tracks["y_vert_disp"], system_tracks["x_vert_disp"])
    ...
    for var in [
        ...
        tilt_dir,
        sys_rel_tilt_dir,
        sys_rel_tilt_dir_alt,
        shear_rel_tilt_dir,
    ]:
        system_tracks = system_tracks.merge(var, left_index=True, right_index=True)
