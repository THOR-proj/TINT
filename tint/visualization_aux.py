...
def add_updrafts(ax, grid, frame_tracks_ind, hgt_ind, ud_ind, projparams, grid_size,
    ...
    for j in updraft_list:
        ...
        ud_height_inds = np.array(frame_tracks_ind['updrafts'][j])
        if max(ud_height_inds[:,0]) >= hgt_ind:
            ...
            ax.scatter(lon_ud, lat_ud, marker='x', s=30, 
...
def plot_vert_winds_line(ax, new_wrf, x_draft_new, y_draft_new, direction, 
                         quiver=False, average_along_line=False, semi_major=None):
...
def get_line_grid_wrf(grid_time, angle, mp='lin'):
    ...
    for var_name in ['U', 'V', 'W']:
        ...
        new_var = xr.Dataset({var_name: (['z', 'y', 'x'],  new_var),},
...
def get_line_grid(f_tobj, grid, uid, nframe):
    ...
    # x is line parallel, y is line perpendicular
    # Little confusing how relative position of dimensions has changed.
    new_grid = xr.Dataset({'reflectivity': (['z', 'y', 'x'],  new_grid),},
                          coords={ 'z': z, 'y': yp, 'x': xp})
