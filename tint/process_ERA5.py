...
def get_ERA5_daterange(year, month):
    ...
    date_range = "{}{}{}-{}{}{}".format(
        year, str(month).zfill(2), "01", year, str(month).zfill(2), last_day
    )
    return date_range
...
def get_ERA5_ds(
    ...
    start_month = np.datetime64(
        "{}-{}".format(str(s_year).zfill(2), str(s_month).zfill(2))
    )
    end_month = np.datetime64(
        "{}-{}".format(str(e_year).zfill(2), str(e_month).zfill(2))
    )
    month_range = np.arange(
        start_month, end_month + np.timedelta64(1, "M"), np.timedelta64(1, "M")
    )
    ...
    for month_dt in month_range:
        ...
        files = collect_ERA5_files(base_dir, year, range_str, files=files)
        CAPE_files.append(
            "{}/cape/{}/cape_era5_oper_sfc_{}.nc".format(
                base_dir.replace("pressure-levels", "single-levels"), year, range_str
            )
        )

    era5_all = xr.open_mfdataset(files, chunks={"longitude": 180})
    CAPE_all = xr.open_mfdataset(CAPE_files, chunks={"longitude": 180})
    if base_timestep != 1:
        ...
        print("Restricting ERA5 data to {} hour timestep".format(base_timestep))
...
def interp_ERA_ds(
    ds_all, CAPE_all, grid, params, timedelta=np.timedelta64(10, "m"), file_list=None
):
    ...
    ds = ds_all.loc[
        dict(
            latitude=slice(max_lat + 0.25, min_lat - 0.25),
            longitude=slice(min_lon - 0.2, max_lon + 0.2),
            time=slice(start_time, end_time),
        )
    ]
    CAPE_ds = CAPE_all.loc[
        dict(
            latitude=slice(max_lat + 0.25, min_lat - 0.25),
            longitude=slice(min_lon - 0.2, max_lon + 0.2),
            time=slice(start_time, end_time),
    ...
    CAPE_ds = CAPE_ds.interp(longitude=lon, latitude=lat, time=times)
    ds = ds.load()
    ...
    cond1 = lr < 2
    cond2 = (
        lr.rolling(dim={"altitude": 4}, min_periods=1).max().shift({"altitude": -3}) < 2
    )
...
def init_ERA5(grid, params, file_list=None):
    ...
    print(
        "Getting interpolated ERA5 for next {} hours.".format(
...
def update_ERA5(grid, params, ERA5_all, CAPE_all, ERA5_interp, file_list=None):
    ...
    grid_datetime_hour = np.datetime64(grid_datetime_hour.replace(second=0, minute=0))
    t_min = ERA5_all.time.values.min()
    t_max = ERA5_all.time.values.max()
    if not (
        grid_datetime >= t_min and grid_datetime_hour + np.timedelta64(1, "h") <= t_max
    ):
        print("Getting ERA5 metadata.")
        ERA5_all, CAPE_all = get_ERA5_ds(
            grid_datetime_hour,
